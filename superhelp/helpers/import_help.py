...
def internal_imports(blocks_dets, *, repeat=False, **_kwargs):
    ...
    brief_msg = layout("""\
        ...
        the value for sys.path inside the module which is failing to import the
        internal module. If it doesn't contain the path to the folder
        surrounding / outside / above the package folder, that is the cause of
        the problem and that is what you need to fix.
        """)
    main_msg = (
        layout("""\
        ...
        2) If absolute importing is not working, don't do "crazy random stuff"
        from Stack Overflow - instead, __check sys.path is correct__. Look at the
        value for sys.path inside the module which is failing to import the
        internal module. If it doesn't contain the path to the folder
        surrounding / outside / above the package folder, that is the cause of
        the problem and that is what you need to fix.

        For example, imagine we have a package called `superhelp` with a module
        `conf.py` and subfolder `displayers`. Iside the subfolder is a module
