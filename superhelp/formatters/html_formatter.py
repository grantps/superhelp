...
""" % {'brief': Level.BRIEF, 'main': Level.MAIN, 'extra': Level.EXTRA}
...
def _get_all_html_strs(code: str, file_path: Path,
    ...
    Suppress overall code display if in notebook - it is right above already
    ...
    if display_snippet:
        overall_snippet_html_strs = repeat_overall_snippet(code, file_path)
...
def get_formatted_help(code: str, file_path: Path, messages_dets, *,
    ...
    if in_notebook:
        formatted_help = NOTEBOOK_HTML_WRAPPER.format(
            head=head,
        ...
        formatted_help = BROWSER_HTML_WRAPPER.format(
