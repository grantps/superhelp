...
## dir(token) = 'Comment', 'Error', 'Escape', 'Generic', 'Keyword', 'Literal', 
## 'Name', 'Number', 'Operator', 'Other', 'Punctuation', 'STANDARD_TYPES', 
## 'String', 'Text', 'Token', 'Whitespace'  ## GPS

## colour codes from http://www.lihaoyi.com/post/BuildyourownCommandLinewithANSIescapecodes.html  ## GPS
MAGENTA = 89  ## GPS
GREEN = 28  ## GPS
RUST = 196  ## GPS
WHITE = 231  ## GPS

code_hl = {  ## GPS reordered
    "Comment": "L",
    "Error": "R",
    "Generic": "CH2",
    "Keyword": "GREEN", # "CH3",  ## GPS I want Green
    "Name": "CH1",
    "Number": "CH4",
    "Operator": "WHITE",  #"CH5",  ## GPS
    "String": "CH4",  ## GPS
    "Operator.Word": "MAGENTA",  ## GPS
    "Literal.String.Single": "RUST",  ## GPS
    "Literal.String.Double": "RUST",  ## GPS  double-quoted string
    "Punctuation": "WHITE",  ## GPS
    "Literal.Number.Integer": "WHITE",  ## GPS
    "Literal.Number.Float": "WHITE",  ## GPS
    "Keyword.Constant": "GREEN",  ## GPS
}
...
if PY3:
    unichr = chr
#     from html.parser import HTMLParser  ## GPS
    from html import unescape  ## GPS  html.parser.HTMLParser.unescape was removed in 3.5
    string_type = str
else:
    from HTMLParser import HTMLParser
    unescape = HTMLParser.unescape  ## GPS
...
def build_hl_by_token():
    ...
    for k, col in list(code_hl.items()):
        ## GPS 
        if '.' not in k:  ## GPS (cope with Operator.Word as k)
            code_hl_tokens[getattr(token, k)] = globals()[col]  ## GPS standard colours defined in this code e.g. CH5
        else:
            ks = k.split('.')
            token2use = token
            for k in ks:
...
#     print(the_html)  ## GPS print a function now
#     return  ## GPS
    ...
    for ph in stash.rawHtmlBlocks:
        nr += 1
#         raw = html_parser.unescape(ph[0])  ## GPS
        raw = unescape(ph)  ## GPS  ## taking slice 0 was a fail given it was a string! Got first character LOL
        if raw[:3].lower() == "<br":
            raw = "\n"
        pre = "<pre><code"
